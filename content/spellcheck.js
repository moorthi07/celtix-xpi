...
function loaded () {
  ...
  try {
    ...
    if (prefBranch.prefHasUserValue('dictionary'))
...
function getLanguages() {
  ...
  for (var i = 0; i < dictionaries.length; i++) {
    try {
      ...
      var splitArray = ab_cd.split('-');
      languages[i] = [languageBundle.getString(splitArray[0].toLowerCase()), ab_cd];
    } catch (ex) {
...
function nextWord () {
  try {
    ...
    if (dialog.word == '') {
      dialog.misspelled.value = dialog.sb.getString('CheckSpellingDone');
    }
...
function updateSuggestions () {
  ...
  if (count == 0) {
    ...
    var none = list.appendItem(dialog.sb.getString('NoSuggestedWords'));
    none.disabled = true;
  }
  else {
